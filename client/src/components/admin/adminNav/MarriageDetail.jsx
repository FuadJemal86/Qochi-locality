...
export default function MarriageDetail() {
    ...
    const [detailMarriageData, setDetailMarriageData] = useState({});
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const result = await api.get(`/admin/get-marriage-detail/${id}`);

                if (result.data.status) {
                    setDetailMarriageData(result.data.getDetailMarriage);
                    setTimeout(() => {
                        setShowImage(true);
                    }, 300);
                } else {
                    console.log(result.data.message);
                    toast.error("Failed to fetch marriage certificate details");
                }
            } catch (err) {
                console.log(err);
                toast.error("An error occurred while fetching marriage certificate details");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const result = await api.post('/admin/update-marriage-document', formData, {
                headers: {
            ...
            if (result.data.status) {
                ...
                const updatedData = await api.get(`/admin/get-marriage-detail/${id}`);
                if (updatedData.data.status) {
                    setDetailMarriageData(updatedData.data.getDetailMarriage);
                }
                setFile(null);
                setFilePreview(null);
            } else {
            ...
            <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden">
                    <div className="p-8 sm:p-16 flex flex-col items-center justify-center">
                        <div className="animate-spin rounded-full h-12 w-12 sm:h-16 sm:w-16 border-t-4 border-b-4 border-red-600"></div>
                        <p className="mt-6 text-gray-600 font-medium">Loading marriage certificate details...</p>
                    </div>
                </div>
            </div>
        );
    ...
    const renderDocument = () => {
        const documentUrl = detailMarriageData?.document ?
            `http://localhost:3032/uploads/members/${detailMarriageData.document}` : null;
        ...
        return (
            <div className="grid grid-cols-1 gap-6">
                <DocumentCard
                    title="Marriage Certificate Document"
    ...
    return (
        <div className="fixed inset-0 bg-opacity-40 flex items-center justify-center z-50 p-0 sm:p-4 backdrop-blur-sm overflow-y-auto">
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full sm:max-w-3xl transform transition-all duration-300 ease-in-out my-0 sm:my-8 max-h-full sm:max-h-[90vh] flex flex-col">
                ...
                <div className="bg-gradient-to-r from-red-500 to-pink-600 py-4 sm:py-6 px-4 sm:px-6 relative flex-shrink-0">
                    <div className="flex justify-between items-center">
                        <h2 className="text-xl sm:text-2xl font-bold text-white">Marriage Certificate Details</h2>
                        <button
                            onClick={closeModal}
                    ...
                    <div className="relative">
                        <div className="flex flex-col sm:flex-row items-center sm:items-start px-4 sm:px-6 pt-6 pb-2">
                            <div className={`transition-all duration-500 ease-in-out ${showImage ? "opacity-100" : "opacity-0"}`}>
                                <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full bg-red-100 flex items-center justify-center border-4 border-white shadow-lg">
                            ...
                            <div className="mt-4 sm:mt-0 sm:ml-4 text-center sm:text-left">
                                <h3 className="text-xl sm:text-2xl font-bold text-gray-800">
                                    {detailMarriageData?.wifeFullName && detailMarriageData?.husbandFullName
                                        ? `${detailMarriageData.wifeFullName} & ${detailMarriageData.husbandFullName}`
                                        : "Marriage Certificate"}
                                </h3>
                                <p className="text-gray-500">
                                    Marriage Certificate
                                    {detailMarriageData?.member?.type ? ` • ${detailMarriageData.member.type}` : ""}
                                </p>
                                <div className="mt-1">
                                    {getStatusBadge(detailMarriageData?.status)}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex overflow-x-auto border-b mt-4 px-4 sm:px-6 no-scrollbar">
                            <button
                                ...
                                className={`px-4 sm:px-6 py-3 font-medium transition-colors relative whitespace-nowrap ${activeTab === "info" ? "text-red-600" : "text-gray-500 hover:text-gray-700"}`}
                            >
                                Marriage Info
                                {activeTab === "info" && (
                            ...
                            <button
                                ...
                                className={`px-4 sm:px-6 py-3 font-medium transition-colors relative whitespace-nowrap ${activeTab === "documents" ? "text-red-600" : "text-gray-500 hover:text-gray-700"}`}
                            >
                                Documents
                                {activeTab === "documents" && (
                                    <span className="absolute bottom-0 left-0 w-full h-0.5 bg-red-600"></span>
                            ...
                            <button
                                ...
                                className={`px-4 sm:px-6 py-3 font-medium transition-colors relative whitespace-nowrap ${activeTab === "upload" ? "text-red-600" : "text-gray-500 hover:text-gray-700"}`}
                            >
                                Upload Document
                                {activeTab === "upload" && (
                                    <span className="absolute bottom-0 left-0 w-full h-0.5 bg-red-600"></span>
                        ...
                        <div className="space-y-6">
                            {activeTab === "info" && (
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                                    <InfoItem
                                        ...
                                        value={detailMarriageData?.wifeFullName || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<FileText className="text-pink-400" size={18} />}
                                        label="Wife's ID"
                                        value={detailMarriageData?.wifeId || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<User className="text-blue-500" size={18} />}
                                        label="Husband's Full Name"
                                        value={detailMarriageData?.husbandFullName || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<FileText className="text-blue-400" size={18} />}
                                        label="Husband's ID"
                                        value={detailMarriageData?.husbandId || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<Calendar className="text-red-500" size={18} />}
                                        label="Date of Marriage"
                                        value={detailMarriageData?.dateOfMarriage ? formatDate(detailMarriageData.dateOfMarriage) : "Not available"}
                                    />
                                    <InfoItem
                                        icon={<Map className="text-green-500" size={18} />}
                                        label="Place of Marriage"
                                        value={detailMarriageData?.placeOfMarriage || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<Map className="text-red-500" size={18} />}
                                        label="Country"
                                        value={detailMarriageData?.country || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<Map className="text-indigo-500" size={18} />}
                                        label="Region"
                                        value={detailMarriageData?.region || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<Map className="text-purple-500" size={18} />}
                                        label="City Administration"
                                        value={detailMarriageData?.cityAdministration || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<Map className="text-teal-500" size={18} />}
                                        label="Zone"
                                        value={detailMarriageData?.zone || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<Map className="text-orange-500" size={18} />}
                                        label="City"
                                        value={detailMarriageData?.city || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<Map className="text-cyan-500" size={18} />}
                                        label="Sub City"
                                        value={detailMarriageData?.subCity || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<Map className="text-blue-400" size={18} />}
                                        label="Woreda"
                                        value={detailMarriageData?.woreda || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<User className="text-green-500" size={18} />}
                                        label="Family Head"
                                        value={detailMarriageData?.familyHead?.fullName || "Not available"}
                                    />
                                    <InfoItem
                                        icon={<Calendar className="text-gray-500" size={18} />}
                                        label="Submission Date"
                                        value={detailMarriageData?.createdAt ? formatDate(detailMarriageData.createdAt) : "Not available"}
                                    />
                                    <InfoItem
                                        icon={<AlertCircle className="text-amber-500" size={18} />}
                                        label="Status"
                                        value={detailMarriageData?.status || "Not available"}
                                    />
                                </div>
                            )}
                            ...
                            {activeTab === "upload" && (
                                <div className="bg-white p-4 rounded-lg border border-gray-200">
                                    <h3 className="text-lg font-semibold text-gray-800 mb-4">Upload Marriage Certificate Document</h3>
                                                    ...
                                                    {!file ? (
                                                        ...
                                                        <div className="space-y-2">
                                                            {filePreview ? (
                                                                ...
                                                                <div className="mx-auto h-16 w-16 bg-red-50 rounded-lg flex items-center justify-center">
                ...
                <div className="border-t p-4 bg-gray-50 flex justify-end flex-shrink-0">
                    <button
                        onClick={closeModal}
                        className="px-5 py-2 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-lg transition-colors shadow-md"
                    >
                        Close
                    </button>
                </div>
...
function InfoItem({ icon, label, value }) {
    return (
        <div className="flex items-start space-x-3 py-2 px-3 rounded-lg hover:bg-red-50 transition-colors">
        ...
        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
            <div className="bg-gray-50 py-3 px-4 border-b">
                <p className="font-medium text-gray-700 flex items-center gap-2">
                    <FileText size={16} className={isPDF ? "text-red-500" : "text-red-500"} />
                    {title}
                </p>
            </div>
