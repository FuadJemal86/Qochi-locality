...
function BirthCertificateApproval() {
    ...
    const filteredData = birthCertificateData.filter(certificate =>
        certificate.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        certificate.fatherFullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        certificate.motherFullName?.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    const handleEdit = (id) => {
        ...
        console.log(`Edit birth certificate with ID: ${id}`);
    };

    const handleDelete = (id) => {
        // Handle delete logic here
        console.log(`Delete birth certificate with ID: ${id}`);
    };
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const result = await api.get('/admin/get-birth-certificate');
            if (result.data.status) {
                setBirthCertificateData(result.data.getBirthCertificate);
            } else {
                console.log(result.data.message);
            }
        } catch (err) {
            console.error('Error fetching birth certificates:', err);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    // print the table
    const handlePrint = () => {
        const printContent = document.getElementById("birth-certificate-table");
        const WindowPrt = window.open('', '', 'width=900,height=650');
        WindowPrt.document.write(`
                <html>
                    <head>
                        <title>Birth Certificate Requests</title>
                        <style>
    ...
    //  export Excel file
    const exportToExcel = () => {
        const ws = XLSX.utils.json_to_sheet(birthCertificateData);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Birth Certificates");

        const excelBuffer = XLSX.write(wb, { bookType: "xlsx", type: "array" });
        const data = new Blob([excelBuffer], { type: "application/octet-stream" });
        saveAs(data, "BirthCertificates.xlsx");
    };

    const handleApproval = async (value, id) => {
        try {
            const result = await api.put(`/admin/birth-certificate-approval/${id}`, {
                ...statusState,
                status: value
            });
        ...
        <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 md:p-6 border border-gray-100">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                    <h1 className="text-2xl font-bold text-gray-800">Birth Certificate Requests</h1>
                ...
                {/* Main table */}
                <div className="border border-gray-200 rounded-lg shadow-sm overflow-hidden" id="birth-certificate-table">
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Place of Birth</th>
                                    <th className="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Father's Name</th>
                                    <th className="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Mother's Name</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                ...
                                ) : birthCertificateData.length > 0 ? (
                                    birthCertificateData.map((certificate) => (
                                        <tr key={certificate.id} className="hover:bg-gray-50 transition-colors duration-150">
                                            ...
                                            {/* Gender */}
                                            <td className="py-4 px-4 whitespace-nowrap text-gray-700">{certificate.gender || "Not specified"}</td>

                                            {/* Place of Birth */}
                                            <td className="py-4 px-4 whitespace-nowrap text-gray-700">{certificate.placeOfBirth || "Not specified"}</td>

                                            {/* Father's Name */}
                                            <td className="py-4 px-4 whitespace-nowrap text-gray-700">{certificate.fatherFullName || "Not specified"}</td>

                                            {/* Mother's Name */}
                                            <td className="py-4 px-4 whitespace-nowrap text-gray-700">{certificate.motherFullName || "Not specified"}</td>
                                            ...
                                            <td className="py-4 px-4 whitespace-nowrap">
                                                <div className="flex space-x-2">
                                                    <Link
                                                        to={`/admin-dashboard/get-detail-birth-certificate/${certificate.id}`}
...
export default BirthCertificateApproval;